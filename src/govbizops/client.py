...
class SAMGovClient:
    ...
    def _check_daily_limit(self):
        ...
        if self._daily_collections >= self.MAX_DAILY_COLLECTIONS:
            raise ValueError(
                f"Daily collection limit reached ({self.MAX_DAILY_COLLECTIONS} per day). Please try again tomorrow."
            )
    ...
    def _validate_naics_codes(self, naics_codes: Optional[List[str]]):
        ...
        if naics_codes and len(naics_codes) > self.MAX_NAICS_CODES:
            raise ValueError(
                f"Maximum {self.MAX_NAICS_CODES} NAICS codes allowed per collection. Got {len(naics_codes)}."
            )
    ...
    def _validate_date_range(self, posted_from: datetime, posted_to: datetime):
        ...
        if days_diff > self.MAX_DAYS_RANGE:
            raise ValueError(
                f"Maximum {self.MAX_DAYS_RANGE} days range allowed per collection. Got {days_diff} days."
            )
    ...
    def search_opportunities(
        ...
        **kwargs,
        ...
        logger.info(
            f"API Response: totalRecords={result.get('totalRecords', 0)}, returned={len(result.get('opportunitiesData', []))}"
        )
