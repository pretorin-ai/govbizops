...
class SAMWebScraper:
    ...
    async def start(self):
        ...
        if not self.playwright:
            ...
            logger.info(
                f"Browser started successfully (server_mode={self.server_mode})"
            )
    ...
    async def scrape_opportunity(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for p in paragraphs:
                ...
                if (
                    len(p_text) > 50
                    and "official website" not in p_text.lower()
                    and "skip to main content" not in p_text.lower()
                    and "sign in" not in p_text.lower()
                    and ".gov means" not in p_text.lower()
                    and "site is secure" not in p_text.lower()
                ):
            ...
            if description_paragraphs:
                ...
                logger.info(
                    f"Found {len(description_paragraphs)} description paragraphs"
                )
            ...
            if not description_text:
                description_patterns = [
                    ...
                    {
                        "name": "section",
                        "attrs": {"aria-label": re.compile("description", re.I)},
                    },
            ...
            attachment_patterns = [
                ...
                {
                    "name": "a",
                    "attrs": {"aria-label": re.compile("download|attachment", re.I)},
                },
            ...
            logger.info(
                f"Successfully scraped opportunity. Description length: {len(description_text) if description_text else 0}"
            )
...
def scrape_sam_opportunity(
    url: str, headless: bool = True, server_mode: bool = None
) -> Dict[str, Any]:
    """
