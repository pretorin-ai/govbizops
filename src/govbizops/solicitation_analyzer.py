...
class SolicitationAnalyzer:
    ...
    def fetch_description_from_api_url(self, desc_url: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"Description URL returned 404. SAM.gov may have changed their API structure."
                )
                logger.warning(
                    "Consider checking SAM.gov's latest API documentation or using the web interface."
                )
                return None
            else:
                logger.warning(
                    f"Failed to fetch description, status: {response.status_code}, response: {response.text[:200]}"
                )
                return None
    ...
    def fetch_description_from_web(
        ...
        try:
            ...
            if result["success"] and result["description"]:
                ...
                logger.warning(
                    f"Web scraping failed: {result.get('error', 'No description found')}"
                )
                return None
    ...
    def fetch_detailed_description(self, opportunity: Dict[str, Any]) -> Optional[str]:
        ...
        for field in description_fields:
            if field in opportunity and opportunity[field]:
                ...
                if (
                    desc_text.startswith("https://api.sam.gov/")
                    and "noticedesc" in desc_text
                ):
                    logger.info(
                        f"Found description URL in field '{field}': {desc_text}"
                    )
                    ...
                    if fetched_content:
                        ...
                        if web_result and web_result["description"]:
                            descriptions.append(
                                f"{field} (from web): {web_result['description']}"
                            )
                            # Store attachments for later use
                            if (
                                "attachments" in web_result
                                and web_result["attachments"]
                            ):
                                if "_web_attachments" not in opportunity:
                                    opportunity["_web_attachments"] = []
                                opportunity["_web_attachments"].extend(
                                    web_result["attachments"]
                                )
                        else:
                            # Both methods failed
                            descriptions.append(
                                f"{field} (URL - fetch failed): {desc_text}"
                            )
        ...
        if descriptions:
            ...
            logger.warning(
                f"No description found in API data for {opportunity.get('noticeId')}"
            )
            return None
    ...
    def extract_documents_info(
        ...
        for pattern in document_patterns:
            ...
            for match in matches:
                ...
                documents.append({"type": match.group(), "context": context})
            ...
            for doc in documents_info[:5]:
                if "name" in doc:
                    documents_text.append(
                        f"{doc.get('type', 'Document')}: {doc.get('name', 'Unknown')} - {doc.get('description', '')}"
                    )
                else:
                    documents_text.append(
                        f"{doc['type']}: {doc.get('context', '')[:200]}"
                    )
...
"""
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a government contracting expert helping to draft solicitation responses.",
                    },
        ...
        if self.openai_client:
            try:
                ai_response = self.generate_ai_response(
                    opportunity, detailed_description, additional_info, documents_info
                )
            except Exception as e:
