...
class TestRunCollector:
    @patch("time.sleep")
    def test_naics_from_args(self, mock_sleep, monkeypatch):
        ...
        with patch("govbizops.main._init_db_session") as mock_init_db:
    ...
    @patch("time.sleep")
    def test_naics_from_env(self, mock_sleep, monkeypatch):
        monkeypatch.setenv("SAM_GOV_API_KEY", "key")
    ...
    @patch("time.sleep")
    def test_naics_default(self, mock_sleep, monkeypatch):
        monkeypatch.setenv("SAM_GOV_API_KEY", "key")
    ...
    @patch("time.sleep")
    def test_limit_validation_naics(self, mock_sleep, monkeypatch):
        monkeypatch.setenv("SAM_GOV_API_KEY", "key")
    ...
    @patch("time.sleep")
    def test_days_back_validation(self, mock_sleep, monkeypatch):
        monkeypatch.setenv("SAM_GOV_API_KEY", "key")
        args = Namespace(
            naics_codes="541511",
            days_back=100,
    ...
    @patch("time.sleep")
    def test_notify_on(self, mock_sleep, monkeypatch):
        monkeypatch.setenv("SAM_GOV_API_KEY", "key")
        monkeypatch.setenv("SLACK_WEBHOOK_URL", "https://hooks.slack.com/test")
        args = Namespace(
            naics_codes="541511",
    ...
    @patch("time.sleep")
    def test_notify_on_no_new_opps(self, mock_sleep, monkeypatch):
        monkeypatch.setenv("SAM_GOV_API_KEY", "key")
        args = Namespace(
            naics_codes="541511",
            days_back=1,
    ...
    @patch("time.sleep")
    def test_notify_off(self, mock_sleep, monkeypatch):
        monkeypatch.setenv("SAM_GOV_API_KEY", "key")
        args = Namespace(
            naics_codes="541511",
            days_back=1,
    ...
    @patch("time.sleep")
    def test_days_back_none(self, mock_sleep, monkeypatch):
...
class TestRunCrmPush:
    ...
    def test_errors_in_result(self, monkeypatch):
        monkeypatch.setenv("CRM_API_KEY", "crm_key")
        args = Namespace(
            crm_url="http://localhost:8000",
            crm_api_key="crm_key",
...
class TestMain:
    @patch("time.sleep")
    def test_collect_mode(self, mock_sleep, monkeypatch):
        monkeypatch.setenv("SAM_GOV_API_KEY", "key")
        with patch(
            "sys.argv",
            ["govbizops", "collect"],
